...
const AppAccordionFlush = {
    data() {
        return {
            accordionTitle: 'Accordion Flush',
            accordionList: [
                {
                    id: 0,
                    title: 'Accordion Item #1',
    ...
    methods: {
        toggleItem(itemId, event){
            let itemContainer = event.target.closest('.accordion__item'),
                itemBodyInner = itemContainer.querySelector('.accordion__item-body-inner'),
                mainContainer = event.target.closest('.accordion'),
                allItemContainers = mainContainer.querySelectorAll('.accordion__item')
            for(let j = 0; j < allItemContainers.length; ++j){
                allItemContainers[j].querySelector('.accordion__item-body').style.height = 0
            }
            if(itemBodyInner && itemBodyInner.clientHeight){
                for(let i in this.accordionList){
                    if(this.accordionList[i].id === itemId){
                        ...
                        if(this.accordionList[i].state === 'opened'){
                            this.accordionList[i].state = 'closed';
                        }
                    }
                }
...
Vue.createApp(AppAccordionFlush).mount('#accordion_flush')